      ...
      <div className="flex items-start gap-3.5 sm:gap-4">
        ...
        >
          ...
          <div className="absolute inset-0 z-40 bg-black/40 opacity-0 group-hover/cover:opacity-100 transition-opacity rounded-r-lg rounded-l-[3px] flex items-center justify-center pointer-events-none backdrop-blur-[1px]">
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold bg-amber-400 text-slate-950 shadow-md transform scale-95 group-hover/cover:scale-100 transition-transform">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
          <div>
            ...
            {book.edition && (
              <div className="mt-1.5 flex items-center gap-1 text-[11px] text-amber-700 dark:text-amber-400 font-medium">
      ...
      <div className="mt-3.5 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          {onSelectForView && (
            <button
              onClick={() => onSelectForView(book)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-emerald-700 hover:bg-emerald-800 text-white transition-colors shadow-sm"
              title="آن لائن پڑھیں"
            >
          ...
          <a
            ...
            download
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-colors"
            title="PDF فائل ڈاؤن لوڈ کریں"
          >
