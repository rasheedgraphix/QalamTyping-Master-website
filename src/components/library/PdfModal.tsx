...
export const PdfModal: React.FC<PdfModalProps> = ({ book, onClose }) => {
  ...
  // Derive preview URL: archive.org download links can be viewed in viewer or directly
  const pdfUrl = book.pdfUrl;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-950/85 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-5xl h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-4 py-3 sm:px-6 sm:py-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 bg-slate-50/90 dark:bg-slate-950/70">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <a
              ...
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-700 hover:bg-emerald-800 text-white transition-colors shadow-sm"
            >
        ...
        <div className="bg-amber-50 dark:bg-amber-950/40 px-4 py-2 border-b border-amber-200/60 dark:border-amber-900/40 text-[12px] text-amber-800 dark:text-amber-300 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <AlertCircle className="w-4 h-4 shrink-0 text-amber-600 dark:text-amber-400" />
            <span>اگر براؤزر میں فائل لوڈ نہ ہو تو براہ راست "ڈاؤن لوڈ PDF" یا "نئی ونڈو" کے بٹن پر کلک کریں۔</span>
          </div>
          <a
            href={pdfUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="underline font-bold shrink-0 hover:text-amber-900 dark:hover:text-amber-200"
          >
            براہ راست کھولیں
          </a>
        </div>
